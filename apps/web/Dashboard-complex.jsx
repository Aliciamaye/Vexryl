  ...
  useEffect(() => {
    const loadUserCreations = async () => {
      try {
        ...
        if (!currentUser) {
          setDashboardError('Please sign in to view your creations');
          setLoading(false);
          return;
        }
          ...
          <div className="flex items-center space-x-4">
            ...
            {currentBot ? (
              <div className="flex items-center space-x-2 bg-green-900/30 px-3 py-1 rounded-lg">
                ...
                <span className="text-sm text-green-400">{currentBot.name || 'Bot Connected'}</span>
              </div>
            ) : (
              <div className="flex items-center space-x-2 bg-yellow-900/30 px-3 py-1 rounded-lg">
                <Bot className="w-4 h-4 text-yellow-500" />
                <span className="text-sm text-yellow-400">No Bot Connected</span>
              </div>
            )}
            ...
            <nav className="flex items-center space-x-2">
              <Link 
                to="/marketplace" 
                className="px-3 py-1 text-gray-300 hover:text-white hover:bg-gray-700 rounded-lg transition-colors"
              >
                Marketplace
              </Link>
              <Link 
      ...
      <div className="flex">
        ...
        <main className="flex-1 bg-gray-900">
          <Routes>
            ...
            <Route path="/commands" element={renderCurrentView()} />
            <Route path="/events" element={renderCurrentView()} />
            <Route path="/settings" element={renderCurrentView()} />
          </Routes>
        </main>
      </div>
